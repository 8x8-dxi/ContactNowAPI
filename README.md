...
api_db() | Alias for the database APIs. Accept a minimum of two params. `$method` is the exposed list of methods on the [database endpoint](https://github.com/8x8-dxi/ContactNowAPI/blob/master/DATABASE.md). `$action` which can be either "create", "update" or "read".  $data = array() | 
